...
<body ng-app="MyApp" ng-controller="MyCtrl">
	...
	<div class="" style="margin:60px 10px">
		<div class="centered" style="margin: auto; max-width: 500px;" ng-init="dapatkanDataAssurance()">
			...
			<table class="table table-center">
				...
				<tbody>
					<tr>
						<td><input type="hidden" ng-model="assuranceText.id_okupansi"/></td>
						<td>
							<select ng-model="assuranceText.nik">
							<option ng-repeat="item in maintain" value="{{item.nik}}">{{item.nik}} || {{item.nama}}</option>
							</select>
						</td>
						<td>
							<select ng-model="assuranceText.id_frame_odp">
							<option ng-repeat="item in dataFrame" value="{{item.id_frame_odp}}">{{item.id_frame_odp}} || {{item.nama_frame}}</option>
							</select>
						</td>	
						<td>
							<input type="submit" class="btn btn-primary" value="SIMPAN" ng-click="simpanDataAssurance()">
					...
					<tr dir-paginate="item in okupansi | filter:s | orderBy:ord:ordval | itemsPerPage:5">
						...
						<td>
							<input type="submit" class="btn btn-primary" value="UBAH" ng-click="ubahAssurance(item,assuranceText)">
							<input type="submit" class="btn btn-primary" value="HAPUS" ng-click="hapusDataAssurance(item.id_okupansi)">
						</td>
					</tr>
				</tbody>
			</table>
